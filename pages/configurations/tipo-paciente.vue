...
const { data: itemsFetch, refresh: itemsRefresh } = await useFetch<General[]>(`${apiURL.value}/tipo-paciente`, { method: 'GET' });
const headers = ref([
...
const deleteItemFetch = async (id: number) => {
    const response = await fetch(
        `${apiURL.value}/tipo-paciente/${id}`, { method: "DELETE" }
    )
    if (response.ok) {
        snackbarStore.setStatus("success", "Eliminado correctamente")
        await itemsRefresh()
    ...
    <v-container fluid v-if="itemsFetch">
        <v-app-bar class="justify-center" flat append>
            <div class="justify-center mx-4">
                ...
                <span class="subheading">Tipo Paciente</span>
            </div>
            <v-spacer></v-spacer>
