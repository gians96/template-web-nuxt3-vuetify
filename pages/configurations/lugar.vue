...
const { data: itemsFetch, refresh: itemsRefresh } = await useFetch<General[]>(`${apiURL.value}/lugar`, { method: 'GET' });
const headers = ref([
...
const deleteItemFetch = async (id: number) => {
    const response = await fetch(
        `${apiURL.value}/lugar/${id}`, { method: "DELETE" }
    )
    if (response.ok) {
        snackbarStore.setStatus("success", "Eliminado correctamente")
        await itemsRefresh()
    ...
    <v-container fluid v-if="itemsFetch">
        <v-app-bar class="justify-center" flat append>
            <div class="justify-center mx-4">
                ...
                <span class="subheading">Lugar</span>
            </div>
            <v-spacer></v-spacer>
