...
const { data: itemsFetch, refresh: itemsRefresh } = await useFetch<General[]>(`${apiURL.value}/interpretacion-atencion`, { method: 'GET' });
const headers = ref([
...
const registerItemFetch = async (item: General) => {
    const response = await fetch(
        `${apiURL.value}/interpretacion-atencion`,
        {
            method: "POST",
            headers: {
...
const deleteItemFetch = async (id: number) => {
    const response = await fetch(
        `${apiURL.value}/interpretacion-atencion/${id}`, { method: "DELETE" }
    )
    if (response.ok) {
        snackbarStore.setStatus("success", "Eliminado correctamente")
        await itemsRefresh()
    ...
    <v-container fluid v-if="itemsFetch">
        <v-app-bar class="justify-center" flat append>
            <div class="justify-center mx-4">
                ...
                <span class="subheading">Interpretación atención</span>
            </div>
            <v-spacer></v-spacer>
                    ...
                    <v-data-table v-model:page="page" v-model="selected" :headers="headers" :items="itemsFetch"
                        ...
                        <template v-slot:item.actions="{ item }">
                            <div class="d-flex justify-center ">
                                <v-btn class="me-2" icon="mdi-pencil" color="yellow" @click="openDialogEditItem(item)">
                                </v-btn>
